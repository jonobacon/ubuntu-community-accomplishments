...
try:
    ...
    if me == None:
        ...
        user = me.name
        teams = [team.name for team in lp.people['ubuntu-beginners-council'].sub_teams]
        if teams == []:
            teams.append(lp.people['ubuntu-beginners-council'].name)
        try:
            memberships = [
                membership for membership in
                lp.people[user].memberships_details
                if membership.team_link.rsplit('~', 1)[-1] in
                    ['ubuntu-beginners-council'] + teams]
        except KeyError:
            memberships = []
        if memberships:
            sys.exit(0)
