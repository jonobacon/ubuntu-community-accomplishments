...
try:
    ...
    if me == None:
        ...
        user = me.name
        teams = [team.name for team in lp.people['ubuntu-membership-board'].sub_teams]
        if teams == []:
            teams.append(lp.people['ubuntu-membership-board'].name)
        try:
            memberships = [
                membership for membership in
                lp.people[user].memberships_details
                if membership.team_link.rsplit('~', 1)[-1] in
                    ['ubuntu-membership-board'] + teams]
        except KeyError:
            memberships = []
        if memberships:
            sys.exit(0)
